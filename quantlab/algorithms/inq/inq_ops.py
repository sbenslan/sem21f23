...
class INQNodeController:
    ...
    def inq_step(self, fraction):
        ...
        # 2) Re-order weights in a zig-zag fashion ->  x[N], x[1], x[N-1], x[2], x[N-2], x[3] ...
            ...
            prev_count = self.weight_frozen.numel() - torch.isnan(self.weight_frozen.data).sum(dtype=torch.long).item()
            new_count = int(self.fraction*self.weight_frozen.numel())
            
            non_quant_idxs = torch.nonzero(torch.isnan(self.weight_frozen.flatten()))[:,0]
            _, idxs = self.weight.flatten()[non_quant_idxs].abs().sort(descending=False)
            zig_zag_idxs = [None]*len(idxs)

            for i in range(len(idxs)):
                zig_zag_idxs[i] = (i%2)*idxs[int(i/2)] + ((i+1)%2)*idxs[len(idxs)-1-int(i/2)]
            idxs = zig_zag_idxs
            length = len(idxs[:new_count-prev_count])
            idxs_sorted = non_quant_idxs[idxs[:new_count-prev_count]]
            idxs_freeze = idxs_sorted[:new_count-prev_count]
            
            self.weight_frozen.data.flatten()[idxs_freeze] = self.inq_quantize(self.weight.data.flatten()[idxs_freeze], quant_levels)
